<%@tag description="基于EasyUI布局的左树+右单grid，支持自动适应窗体大小变化" pageEncoding="UTF-8"%>
...
<div class="easyui-layout" data-options="fit:true">
  ...
  <div data-options="region:'center',border:false">
    <div class="easyui-layout" data-options="fit:true">
      ...
      <div data-options="region:'center',border:false" style="padding: 5px 10px 5px 8px;">
        <g:grid listFields="${listFields}" gridId="${gridId}" listFrozenFields="${listFrozenFields}" defaultOrder="${defaultOrder}" listUrl="${listUrl}" functions="${functions}"
          isHandleDblClickRow="${isHandleDblClickRow}" isPage="${isPage}" isSingleSelect="${isSingleSelect}" dblClickRowHandler="${dblClickRowHandler}" selectHandler="${selectHandler}" clickCellHandler="${clickCellHandler}"
        />
      </div>
    </div>
