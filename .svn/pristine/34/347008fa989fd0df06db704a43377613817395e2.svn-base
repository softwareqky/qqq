...
import project.edge.dao.quality.QualityReportAttachmentDao;
import project.edge.dao.quality.QualityReportDao;
...
/**
 * @author angel_000
 *         [t_quality_report]对应的 Service。
 */
@Service
public class QualityReportServiceImpl extends GenericServiceImpl<QualityReport, String>
    ...
    @Resource
    private QualityReportAttachmentDao qualityReportAttachmentDao;

    @Override
    public Dao<QualityReport, String> getDefaultDao() {
        return this.qualityReportDao;
    }
    ...
    @Override
    @Transactional
    public void create(QualityReport entity) {
...
//        if (this.archiveDao.find(FolderPath.QUALITY_REPORT) == null) {
...
//        }
        ...
        if(this.archiveDao.find(entity.getProject().getId() + "_" + FolderPath.QUALITY_REPORT) == null) {
            this.archiveDao.create(this.getQualityReportRootDirArchive(year,entity));
        }
        
        this.qualityReportDao.create(entity);
        for (QualityReportAttachment a : entity.getQualityReportAttachments()) {
            this.archiveDao.create(a.getArchive());
            this.qualityReportAttachmentDao.create(a);
        }
    }
    ...
    @Override
    @Transactional
    public void update(QualityReport entity) {
        if (entity == null || StringUtils.isEmpty(entity.getId())) {
            return;
        }
...
//        }
        ...
        // 创建新增的文件
        for (QualityReportAttachment attachment : entity.getQualityReportAttachments()) {

            this.archiveDao.create(attachment.getArchive());
            this.qualityReportAttachmentDao.create(attachment);
        }

        super.update(entity);


        for (QualityReportAttachment attachmentToDelete : entity.getAttachmentsToDelete()) {
...
//        }
     ...
     * @return
     */
    private Archive getQualityReportRootDirArchive(String year, QualityReport entity) {

        // 实体对应的文件夹Archive
        Archive dir = new Archive();
        dir.setId(entity.getProject().getId() + "_" + FolderPath.QUALITY_REPORT);
        dir.setArchiveName("质量上报"); // 项目名不允许修改，所以不考虑变更项目名引起项目文件夹改名的问题
        dir.setIsDir(OnOffEnum.ON.value());
...
//        }
     ...
     * @return
     */
    private Archive getDirArchive(QualityReport entity) {

        // 实体对应的文件夹Archive
        Archive dir = new Archive();
        dir.setId(entity.getId());
        dir.setArchiveName(entity.getProject().getProjectName() + "_" + entity.getReportCode());
        dir.setIsDir(OnOffEnum.ON.value());
        dir.setRelativePath(File.separator + FolderPath.QUALITY_REPORT + File.separator + dir.getId());
        dir.setFileSize(null);
        dir.setPid(FolderPath.QUALITY_REPORT);
         ...
         */
        dir.setLevel(2);
        dir.setPath(Constants.SLASH + Constants.COMMA + FolderPath.QUALITY_REPORT);
        dir.setIsDeleted(OnOffEnum.OFF.value());
    ...
    private Archive getProjectRootDirArchive() {
        ...
        // 实体对应的文件夹Archive
        Archive dir = new Archive();
        dir.setId(FolderPath.QUALITY_REPORT);
        dir.setArchiveName("质量上报归档");
        dir.setIsDir(OnOffEnum.ON.value());
        dir.setRelativePath(File.separator + dir.getId());
        dir.setFileSize(null);
        dir.setPid(Constants.SLASH);
