...
import project.edge.dao.budget.BudgetTemplateDao;
 ...
 */
@Service
public class BudgetTemplateServiceImpl extends GenericServiceImpl<BudgetTemplate, String>
    ...
    @Override
    public Dao<BudgetTemplate, String> getDefaultDao() {
        // TODO Auto-generated method stub
        return this.budgetTemplateDao;
    }
    ...
    @Override
    @Transactional
    public void create(BudgetTemplate entity) {
...
//        }
        ...
        if(this.archiveDao.find(entity.getProject().getId() + "_" + FolderPath.BUDGET_TEMPLATE) == null) {
            this.archiveDao.create(this.getBudgetTemplateRootDirArchive(year,entity));
        }
        
        this.budgetTemplateDao.create(entity);
        for (BudgetTemplateAttachment a : entity.getArchives()) {
            this.archiveDao.create(a.getArchive());
            this.budgetTemplateAttachmentDao.create(a);
        }
    }
    ...
    @Override
    @Transactional
    public void update(BudgetTemplate entity) {
        if (entity == null || StringUtils.isEmpty(entity.getId())) {
            return;
        }
...
//        }
        ...
        for (BudgetTemplateAttachment attachmentToDelete : entity.getArchivesToDelete()) {
...
//        }
     ...
     * @return
     */
    private Archive getBudgetTemplateRootDirArchive(String year, BudgetTemplate entity) {

        // 实体对应的文件夹Archive
        Archive dir = new Archive();
        dir.setId(entity.getProject().getId() + "_" + FolderPath.BUDGET_TEMPLATE);
        dir.setArchiveName("预算模板"); // 项目名不允许修改，所以不考虑变更项目名引起项目文件夹改名的问题
        dir.setIsDir(OnOffEnum.ON.value());
...
//        }
     ...
     * @return
     */
    private Archive getDirArchive(BudgetTemplate entity) {

        // 实体对应的文件夹Archive
        Archive dir = new Archive();
        dir.setId(entity.getId());
        dir.setArchiveName(entity.getTemplateName());
        dir.setIsDir(OnOffEnum.ON.value());
        dir.setRelativePath(
                File.separator + FolderPath.BUDGET_TEMPLATE + File.separator + dir.getId());
        dir.setFileSize(null);
        dir.setPid(FolderPath.BUDGET_TEMPLATE);
         ...
         */
        dir.setLevel(2);
        dir.setPath(Constants.SLASH + Constants.COMMA + FolderPath.BUDGET_TEMPLATE);
        dir.setIsDeleted(OnOffEnum.OFF.value());
