...
/**
 * @author angel_000 [t_project_person_change]对应的DAO。
 */
@Repository
public class ProjectPersonChangeDaoImpl extends HibernateDaoImpl<ProjectPersonChange, String>
        implements ProjectPersonChangeDao {

    @Override
    public List<ProjectPersonChange> list(DataFilter filter, List<OrderByDto> orders,
            PageCtrlDto page) {
     ...
     */
    @Override
    public ProjectPersonChange find(String id) {
        if (StringUtils.isEmpty(id)) {
            return null;
        }

        // 这里将关联的信息全部取出，主要是为了配合新建/修改后重新获取实体信息，然后将这个完整的实体信息显示到一览画面中
        ProjectPersonChange projectPersonChange = (ProjectPersonChange) session()
