...
/**
 * [t_contract_account]对应的DAO。
 */
@Repository
public class ContractAccountDaoImpl extends HibernateDaoImpl<ContractAccount, String>
        implements ContractAccountDao {

    @Override
    public List<ContractAccount> list(DataFilter filter, List<OrderByDto> orders,
            PageCtrlDto page) {
     ...
     */
    @Override
    public ContractAccount find(String id) {
        if (StringUtils.isEmpty(id)) {
            return null;
        }

        // 这里将关联的信息全部取出，主要是为了配合新建/修改后重新获取实体信息，然后将这个完整的实体信息显示到一览画面中
        ContractAccount contractAccount = (ContractAccount) session().createCriteria(this.type)
