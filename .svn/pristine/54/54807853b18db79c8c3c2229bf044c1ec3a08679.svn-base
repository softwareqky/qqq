        ...
        var PROJECTINSPECT = (function($) {
        	...
        	function openEditExpertReviewFormDialog(gridId, url) {
                ...
                if (selectedRows.length == 1) {
                    status = selectedRows[0].inspectStatus_;
                } else {
                ...
                if (status == 'INSPECT_STATUS_0') { // 待审批
            ...
            function openEditFormDialog(gridId, url) {
                ...
                var dialogId = "#${idMap['edit-form-dialog']}";
                if (status == 'INSPECT_STATUS_2') { // 进展中->整改
                    dialogId += '-Improve';
                } else if (status == 'INSPECT_STATUS_1') { // 上报->验证
            ...
            function handleSelect(index, row) {
                ...
                if (rows.length == 1) {
                    for (var i = 0; i < rows.length; i++) {
                    	...
                    	if (rows[i].inspectStatus_ == 'INSPECT_STATUS_0' && rows[i].flowStatus == 2) {
                    		// 巡查状态：发起， 流程状态：审核通过
                    		expertSelFunc = 'enable';
                    		expertReviewFunc = 'enable';
                            break;
                    	} else if (rows[i].inspectStatus_ == 'INSPECT_STATUS_2') { // 整改
                        	improveFunc = 'enable';
                            break;
                        } else if (rows[i].inspectStatus_ == 'INSPECT_STATUS_1') { // 验证
            ...
            function openSelExpertDialog() {
                ...
                function beforeOpen(isFirstLoad) {
                    ...
                    options.queryParams.projectInspectId = recordId;
