...
<body>
  ...
  <g:treeChartEasyuiLayout idPrefix="${idPrefix}" chartUrl="${urlMap['chart']}" chartId="${idMap['main-chart']}">
    <c:choose>
      <c:when test="${'PROJECT' == sideDataType}">
        <g:sideProjectFilterTree idPrefix="${idPrefix}" treeUrl="${urlMap['tree-side-project']}" linkedFunc="${jsMap['linked-func']}" />
      </c:when>
    </c:choose>
  </g:treeChartEasyuiLayout>
