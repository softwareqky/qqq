...
/**
 * @author angel_000
 *         [t_notice_attachment]对应的DAO。
 */
@Repository
public class NoticeAttachmentDaoImpl
        extends HibernateDaoImpl<NoticeAttachment, String>
        implements NoticeAttachmentDao {
	@Override
    protected DetachedCriteria prepareConditions(DetachedCriteria criteria, DataFilter filter) {

        criteria.createAlias("notice", "notice", JoinType.LEFT_OUTER_JOIN)
     ...
     */
    @Override
    public NoticeAttachment find(String id) {
        ...
        NoticeAttachment noticeAttachment =
                (NoticeAttachment) session().createCriteria(this.type)
                        .createAlias("notice", "notice", JoinType.LEFT_OUTER_JOIN)
                        .createAlias("archive", "archive", JoinType.LEFT_OUTER_JOIN)
