...
<%@attribute name="auditSubmitJs" required="true" description="新建按钮的JavaScript处理函数"%>
...
<c:if test="${empty dialogTitle}">
  <s:message var="dialogTitle" code="ui.common.submit.audit" />
</c:if>
<c:if test="${empty dialogIconClass}">
  <c:set var="dialogIconClass" value="icon-func-submit-audit" />
</c:if>
...
<div class="hidden">
  <div id="${dialogId}" class="easyui-dialog"
    data-options="title:'${dialogTitle}',closed:true,buttons:'#${dialogId}Buttons',iconCls:'${dialogIconClass}',<c:if test="${not empty height}">height:${height},</c:if>
            maxHeight:${maxHeight},width:550,modal:true,shadow:${isShadow},resizable:false,onMove:EasyDialogUtils.onDialogMove"
  >
    <jsp:doBody />
  </div>
  ...
  <div id="${dialogId}Buttons">
    ...
    <a href="javascript:void(0)" class="easyui-linkbutton c8" role="button" onclick="${auditSubmitJs}"> <s:message code="ui.common.ok" />
    </a> <a href="javascript:void(0)" class="easyui-linkbutton" role="button" onclick="EasyDialogUtils.closeFormDialog('#${dialogId}');"> <s:message code="ui.common.quit" />
    </a>
  </div>
</div>