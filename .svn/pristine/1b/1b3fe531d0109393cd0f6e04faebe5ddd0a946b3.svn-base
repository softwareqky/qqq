        ...
        var PROJECTCHECK = (function($) {
            ...
            function handleSelect(index, row) {
                ...
                if (rows.length == 1) {
                    for (var i = 0; i < rows.length; i++) {
                    	...
                    	//if (rows[i].flowStatus == '0')
                    	if (rows[i].checkStatus_ == 'CHECK_STATUS_0' && rows[i].flowStatus == 2) { // 待审批
                    		expertSelFunc = 'enable';
                    		expertReviewFunc = 'enable';
                            break;
                    	} else if (rows[i].checkStatus_ == 'CHECK_STATUS_1') { // 待整改
                        	improveFunc = 'enable';
                            break;
                        } else if (rows[i].checkStatus_ == 'CHECK_STATUS_2') { // 待验证
            ...
            function openSelExpertDialog() {
                ...
                function beforeOpen(isFirstLoad) {
                    ...
                    options.queryParams.projectCheckId = recordId;
