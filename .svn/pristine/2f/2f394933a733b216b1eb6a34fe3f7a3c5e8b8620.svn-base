...
import project.edge.dao.quality.QualityExamineAttachmentDao;
import project.edge.dao.quality.QualityExamineDao;
...
/**
 * @author angel_000
 *         [t_quality_examine]对应的 Service。
 */
@Service
public class QualityExamineServiceImpl extends GenericServiceImpl<QualityExamine, String>
        implements QualityExamineService {

    @Resource
    private QualityExamineDao qualityExamineDao;
    
    @Resource
    private ArchiveDao archiveDao;
    
    @Resource
    private QualityExamineAttachmentDao qualityExamineAttachmentDao;

    @Override
    public Dao<QualityExamine, String> getDefaultDao() {
        return this.qualityExamineDao;
    }
    ...
    @Override
    @Transactional
    public void create(QualityExamine entity) {
...
//        if (this.archiveDao.find(FolderPath.QUALITY_EXAMINE) == null) {
...
//        }
        ...
        if(this.archiveDao.find(entity.getProject().getId() + "_" + FolderPath.QUALITY_EXAMINE) == null) {
            this.archiveDao.create(this.getQualityExamineRootDirArchive(year,entity));
        }
        
        this.qualityExamineDao.create(entity);
        for (QualityExamineAttachment a : entity.getQualityExamineAttachments()) {
            this.archiveDao.create(a.getArchive());
            this.qualityExamineAttachmentDao.create(a);
        }
    }
    ...
    @Override
    @Transactional
    public void update(QualityExamine entity) {
        if (entity == null || StringUtils.isEmpty(entity.getId())) {
            return;
        }
...
//        }
        ...
        // 创建新增的文件
        for (QualityExamineAttachment attachment : entity.getQualityExamineAttachments()) {

            this.archiveDao.create(attachment.getArchive());
            this.qualityExamineAttachmentDao.create(attachment);
        }

        super.update(entity);


        for (QualityExamineAttachment attachmentToDelete : entity.getAttachmentsToDelete()) {
...
//        }
     ...
     * @return
     */
    private Archive getQualityExamineRootDirArchive(String year, QualityExamine entity) {

        // 实体对应的文件夹Archive
        Archive dir = new Archive();
        dir.setId(entity.getProject().getId() + "_" + FolderPath.QUALITY_EXAMINE);
        dir.setArchiveName("质量考核"); // 项目名不允许修改，所以不考虑变更项目名引起项目文件夹改名的问题
        dir.setIsDir(OnOffEnum.ON.value());
...
//        }
     ...
     * @return
     */
    private Archive getDirArchive(QualityExamine entity) {

        // 实体对应的文件夹Archive
        Archive dir = new Archive();
        dir.setId(entity.getId());
        dir.setArchiveName(entity.getProject().getProjectName() + "_" + entity.getExamineCode());
        dir.setIsDir(OnOffEnum.ON.value());
        dir.setRelativePath(File.separator + FolderPath.QUALITY_EXAMINE + File.separator + dir.getId());
        dir.setFileSize(null);
        dir.setPid(FolderPath.QUALITY_EXAMINE);
         ...
         */
        dir.setLevel(2);
        dir.setPath(Constants.SLASH + Constants.COMMA + FolderPath.QUALITY_EXAMINE);
        dir.setIsDeleted(OnOffEnum.OFF.value());
    ...
    private Archive getProjectRootDirArchive() {
        ...
        // 实体对应的文件夹Archive
        Archive dir = new Archive();
        dir.setId(FolderPath.QUALITY_EXAMINE);
        dir.setArchiveName("质量考核归档");
        dir.setIsDir(OnOffEnum.ON.value());
        dir.setRelativePath(File.separator + dir.getId());
        dir.setFileSize(null);
        dir.setPid(Constants.SLASH);
