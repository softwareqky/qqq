 ...
 */
@Repository
public class TenderingPlanRelatedunitDaoImpl
        extends HibernateDaoImpl<TenderingPlanRelatedunit, String>
        implements TenderingPlanRelatedunitDao {
    @Override
    protected DetachedCriteria prepareConditions(DetachedCriteria criteria, DataFilter filter) {

        criteria.createAlias("tenderingPlan", "tenderingPlan", JoinType.LEFT_OUTER_JOIN)
        	     .createAlias("relatedUnit", "relatedUnit", JoinType.LEFT_OUTER_JOIN)
     ...
     */
    @Override
    public TenderingPlanRelatedunit find(String id) {
        if (StringUtils.isEmpty(id)) {
            return null;
        }

        // 这里将关联的信息全部取出，主要是为了配合新建/修改后重新获取实体信息，然后将这个完整的实体信息显示到一览画面中
        TenderingPlanRelatedunit tenderingPlanRelatedunit =
                (TenderingPlanRelatedunit) session().createCriteria(this.type)
