...
/**
 * [t_prequalification]对应的DAO。
 */
@Repository
public class PrequalificationDaoImpl extends HibernateDaoImpl<Prequalification, String>
        implements PrequalificationDao {

    @Override
    public List<Prequalification> list(DataFilter filter, List<OrderByDto> orders,
            PageCtrlDto page) {
     ...
     */
    @Override
    public Prequalification find(String id) {
        if (StringUtils.isEmpty(id)) {
            return null;
        }

        // 这里将关联的信息全部取出，主要是为了配合新建/修改后重新获取实体信息，然后将这个完整的实体信息显示到一览画面中
        Prequalification prequalification = (Prequalification) session().createCriteria(this.type)
