...
/**
 * 用于弹出选择画面的共通的Controller，典型的画面包含左侧的tree、中间的备选DataGrid和右侧的选择结果DataGrid，三者联动。其中中间的datagrid通常是检索用，约定为SubGrid，右侧的datagrid为选择结果MainGrid。
 * 
 ...
 * @param <V> 表现层实体
 */
public abstract class TreeDoubleGridSelectorController<T, V extends ViewBean>
        extends TreeDoubleGridUploadController<T, V> {

    protected static final String TREE_GRID_SELECTOR_VIEW_NAME =
            "common/page/treeDoubleGridSelectorPage"; // 默认的JSP
    ...
    public String getGridTitle(Locale locale) {
        return this.messageSource.getMessage("ui.grid.title.selection.result", null, locale);
    }
