...
import project.edge.dao.process.ProjectCheckDao;
...
/**
 * @author angel_000
 *         [t_project_check]对应的Service。
 */
@Service
public class ProjectCheckServiceImpl extends GenericServiceImpl<ProjectCheck, String>
        implements ProjectCheckService {
    ...
    @Override
    public Dao<ProjectCheck, String> getDefaultDao() {
        // TODO Auto-generated method stub
        return this.projectCheckDao;
    }
    ...
    @Override
    @Transactional
    public void create(ProjectCheck entity) {
...
//        if (this.archiveDao.find(FolderPath.PROJECT_CHECK) == null) {
...
//        }
        ...
        if(this.archiveDao.find(entity.getProject().getId() + "_" + FolderPath.PROJECT_CHECK) == null) {
            this.archiveDao.create(this.getProjectCheckRootDirArchive(year,entity));
        }
        
        this.projectCheckDao.create(entity);
        for (ProjectCheckAttachment a : entity.getProjectCheckAttachments()) {
            this.archiveDao.create(a.getArchive());
            this.projectCheckAttachmentDao.create(a);
        }
    ...
    @Override
    @Transactional
    public void update(ProjectCheck entity) {
        if (entity == null || StringUtils.isEmpty(entity.getId())) {
            return;
        }
...
//        }
        ...
        // 创建新增的文件
        for (ProjectCheckAttachment attachment : entity.getProjectCheckAttachments()) {

            this.archiveDao.create(attachment.getArchive());
            this.projectCheckAttachmentDao.create(attachment);
        }

        super.update(entity);

        for (ProjectCheckAttachment attachmentToDelete : entity.getAttachmentsToDelete()) {
            this.archiveDao.delete(attachmentToDelete.getArchive());
        }
    }
    
    @Transactional
    public void setData(ProjectCheck entity) {
    	if (entity == null || StringUtils.isEmpty(entity.getId())) {
            return;
        }
    	super.update(entity);
...
//        }
     ...
     * @return
     */
    private Archive getProjectCheckRootDirArchive(String year, ProjectCheck entity) {

        // 实体对应的文件夹Archive
        Archive dir = new Archive();
        dir.setId(entity.getProject().getId() + "_" + FolderPath.PROJECT_CHECK);
        dir.setArchiveName("项目检查"); // 项目名不允许修改，所以不考虑变更项目名引起项目文件夹改名的问题
        dir.setIsDir(OnOffEnum.ON.value());
...
//        }
     ...
     * @return
     */
    private Archive getDirArchive(ProjectCheck entity) {
        ...
        dir.setIsDir(OnOffEnum.ON.value());
        dir.setRelativePath(
                File.separator + FolderPath.PROJECT_CHECK + File.separator + dir.getId());
        dir.setFileSize(null);
        dir.setPid(FolderPath.PROJECT_CHECK);
         ...
         */
        dir.setLevel(2);
        dir.setPath(Constants.SLASH + Constants.COMMA + FolderPath.PROJECT_CHECK);
        dir.setIsDeleted(OnOffEnum.OFF.value());
    ...
    private Archive getProjectRootDirArchive() {
        ...
        // 实体对应的文件夹Archive
        Archive dir = new Archive();
        dir.setId(FolderPath.PROJECT_CHECK);
        dir.setArchiveName("项目检查归档");
        dir.setIsDir(OnOffEnum.ON.value());
        dir.setRelativePath(File.separator + dir.getId());
        dir.setFileSize(null);
        dir.setPid(Constants.SLASH);
