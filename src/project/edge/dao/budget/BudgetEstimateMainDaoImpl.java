...
/**
 * @author wwy
 *         [t_budget_final]对应的DAO。
 */
@Repository
public class BudgetEstimateMainDaoImpl extends HibernateDaoImpl<BudgetEstimateMain, String>
        implements BudgetEstimateMainDao {
     ...
     */
    @Override
    public BudgetEstimateMain find(String id) {
        if (StringUtils.isEmpty(id)) {
            return null;
        }

        // 这里将关联的信息全部取出，主要是为了配合新建/修改后重新获取实体信息，然后将这个完整的实体信息显示到一览画面中
        BudgetEstimateMain b = (BudgetEstimateMain) session().createCriteria(this.type)
