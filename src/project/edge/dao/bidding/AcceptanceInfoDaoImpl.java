...
/**
 * [t_acceptance_info]对应的DAO。
 */
@Repository
public class AcceptanceInfoDaoImpl extends HibernateDaoImpl<AcceptanceInfo, String>
        implements AcceptanceInfoDao {

    @Override
    public List<AcceptanceInfo> list(DataFilter filter, List<OrderByDto> orders, PageCtrlDto page) {
        return getList(filter, orders, page);
    }

    @Override
    public List<AcceptanceInfo> list(DataFilter filter, List<OrderByDto> orders) {
     ...
     */
    @Override
    public AcceptanceInfo find(String id) {
        if (StringUtils.isEmpty(id)) {
            return null;
        }

        // 这里将关联的信息全部取出，主要是为了配合新建/修改后重新获取实体信息，然后将这个完整的实体信息显示到一览画面中
        AcceptanceInfo acceptanceInfo = (AcceptanceInfo) session().createCriteria(this.type)
