...
/**
 * [t_project_person_change_detail]对应的DAO。
 */
@Repository
public class ProjectPersonChangeDetailDaoImpl
        extends HibernateDaoImpl<ProjectPersonChangeDetail, String>
        implements ProjectPersonChangeDetailDao {
    ...
    protected DetachedCriteria prepareConditions(DetachedCriteria criteria, DataFilter filter) {
        ...
        criteria.createAlias("project", "project", JoinType.LEFT_OUTER_JOIN)
                ...
                .createAlias("projectPersonChange", "projectPersonChange", JoinType.LEFT_OUTER_JOIN)
     ...
     */
    @Override
    public ProjectPersonChangeDetail find(String id) {
        if (StringUtils.isEmpty(id)) {
            return null;
        }

        // 这里将关联的信息全部取出，主要是为了配合新建/修改后重新获取实体信息，然后将这个完整的实体信息显示到一览画面中
        ProjectPersonChangeDetail projectPersonDetail = (ProjectPersonChangeDetail) session()
